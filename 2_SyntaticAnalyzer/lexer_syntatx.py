...
class Lexer:
    ...
    def handle_operator(self, op_src):
        ...
        if op_src in '=!<>':
            ...
            if next_char in '=<>':
                ...
                # HANA does not support ++ or --
                value += next_char
            else:
                self.position -= 1
                self.column -= 1
        
        # Check if the operator is followed by an appropriate token
        next_char = self.lookahead()
    ...
    def handle_string(self, str_src):
        ...
        while True:
            char = self.lookahead()
            if char is None:
                return Token(TokenType.ERROR, f"Unterminated string  at line {self.line} column {self.column}")
            value += char
            if char == '"':
                break
            ...
            if self.hana_delimiter:
                ...
                if (last_delimiter == '(' and delimiter == ')') or \
                   ...
                   (last_delimiter == '[' and delimiter == ']'):
                    ...
                    return Token(TokenType.ERROR, f"Mismatched delimiter: expected closing {last_delimiter}, found {delimiter} at line {self.line} column {self.column}")
            else:
                return Token(TokenType.ERROR, f"Unmatched closing delimiter: {delimiter} at line {self.line} column {self.column}")
    ...
    def handle_identifier(self, id_src):
        ...
        if value in self.hana_keywords:
            ...
            token = Token(TokenType.OPERATOR, value)
        else:
            token = Token(TokenType.IDENTIFIER, value)

            # Check for spaces followed by another identifier without an operator or delimiter
            next_char = self.lookahead()
            ...
            if next_char is not None and next_char.isalnum():
                if value in ":,":
                    ...
                    return Token(TokenType.ERROR, f"Unexpected identifier sequence: '{value} {next_char}' at line {self.line} column {self.column}")
