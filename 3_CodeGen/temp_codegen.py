...
class MIPSCodeGenerator:
    ...
    def handle_func_def_node(self, node):
        ...
        self.code.append("addi $sp, $sp, -4  # Reserve stack space for $ra")
        self.code.append("sw $ra, 0($sp)    # Save return address")
        for stmt in node.body:
            self.process_ast(stmt)
        self.code.append("lw $ra, 0($sp)    # Restore return address")
        self.code.append("addi $sp, $sp, 4  # Free stack space")
        self.code.append("jr $ra            # Return from function")
    
    def handle_func_call_node(self, node):
        for i, arg in enumerate(node.args):
            self.process_ast(arg)
            self.code.append(f"sw $v0, -{(i + 1) * 4}($sp)  # Save arg {i} to stack")
        self.code.append(f"jal {node.func_name}  # Call function {node.func_name}")
        self.code.append(f"addi $sp, $sp, {len(node.args) * 4}  # Free arg space on stack")
    ...
    def handle_assign_node(self, node):
        ...
        stack_offset = self.get_stack_offset(node.var.name)
        self.code.append(f"sw $v0, {stack_offset}($sp)  # Store value in stack")
    ...
    def handle_if_node(self, node):
        ...
        self.code.append(f"beq $v0, $zero, {false_label}  # If condition is false, jump to {false_label}")
        for stmt in node.body:
            self.process_ast(stmt)
        self.code.append(f"j {end_label}  # Skip else block")
        self.code.append(f"{false_label}:")
        if node.else_body:
            for stmt in node.else_body:
                self.process_ast(stmt)
